...
BOOL CDlgLogin::ConnectServer()
{
	...
	//-----------------------------------------------------------------------------
	// 서버접속
	BOOL bResult1 = g_iXingAPI1.Connect( 
		AfxGetMainWnd()->GetSafeHwnd(),			// Connect가 된 이후에 Disconnect 메시지를 받을 윈도우 Handle
												// Login 윈도우는 Login 만 처리하는 윈도우 이므로 Disconnect는 메인윈도우에서 받는다.
...
BOOL CDlgLogin::Login()
{
	//-----------------------------------------------------------------------------
	// 아이디
	CString strID; GetDlgItemText( IDC_EDIT_ID, strID );

	//-----------------------------------------------------------------------------
	// 비밀번호
	CString strPwd; GetDlgItemText( IDC_EDIT_PWD, strPwd );
	
	//-----------------------------------------------------------------------------
	// 공인인증 비밀번호
	CString strCertPwd; GetDlgItemText( IDC_EDIT_CERT_PWD, strCertPwd );
...
void CDlgLogin::SetServerIP( LPCTSTR pszAddrType, LPCTSTR pszServerIP )
{
	...
	else if( strcmp( pszAddrType, "모의투자"  ) == 0 ) strIP = "demo.ebestsec.co.kr";
